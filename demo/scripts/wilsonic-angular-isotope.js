        ...
        .controller("angularIsotopeController", [
            "iso.config", "iso.topics", "$scope", "$timeout", "optionsStore", function(config, topics, $scope, $timeout, optionsStore) {
                        ...
                        if (!(window.jQuery && isotopeContainer.isotope(opts)))
                        {
                            ...
                            isotopeContainer.isotope = function(options, callback) {
                                ...
                                if ( typeof options === 'string' ) {
                                    if(options == 'remove'){
                                       var rtrn =  instance[options].apply(instance, args);
                                        $scope.refreshIso()
                                        return rtrn;
                                    }else{
                ...
                $scope.setIsoElement = function($element) {
                    if (postInitialized) {
                        return $timeout(function() {
                            return angular.element(isotopeContainer).isotope($scope.isoMode, $element);
                        });
                    }
                };
                $scope.refreshIso = function() {
                    if (postInitialized) {
                        return angular.element(isotopeContainer).isotope();
                    }
                };
                $scope.updateOptions = function(option) {
                    if (angular.element(isotopeContainer)) {
                        angular.element(isotopeContainer).isotope(option);
                    } else {
                        optionsStore.store(option);
                    }
                };
                $scope.updateMethod = function(name, params, cb) {
                    return angular.element(isotopeContainer).isotope(name, params, cb);
                };
                ...
                $scope.removeAll = function(cb) {
                    return angular.element(isotopeContainer).isotope("remove", angular.element(isotopeContainer).data("isotope").$allAtoms, cb);
                };
                ...
                $scope.removeElement = function(element) {
                    return angular.element(isotopeContainer) && angular.element(isotopeContainer).isotope("remove", element);
                };
            }
        ])
